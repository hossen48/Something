...
function openChapter(name){opened.add(name);updateDone();showTemplate(map[name])}
function showTemplate(id){$('#modalContent').innerHTML='';$('#modalContent').appendChild(document.getElementById(id).content.cloneNode(true));$('#modal').classList.add('show');document.body.style.overflow='hidden';bindDynamic(id)}
function bindDynamic(id){
  ...
  $('#finalClose')?.addEventListener('click',()=>closeModal());
}
...
function bindQuiz(){const qs=[['Who is more dramatic?',['Adu 😇','Hossen 😌','Both. Obviously.'],2],['Who has the evil creepy laugh?',['Adu','Hossen','The neighbours'],1],['What happened on 24 January 2026?',['A random Tuesday','The relationship became official','A software update'],1],['What does Hossen secretly love even when it annoys him?',['Adu’s anger','His own jokes','Traffic'],0],['Where is a favourite date memory?',['Ramna','The moon','The kitchen sink'],0],['What is the official birthday today?',['12 September','24 January','Sometime in winter'],0],['Who is Hossen trying to marry as soon as possible?',['Adu','His Wi-Fi router','Nobody, this is a trap'],0]];let i=0,score=0;const q=$('#quizQuestion'),ans=$('#quizAnswers'),res=$('#quizResult'),prog=$('#quizProgress'),scoreEl=$('#quizScore');function render(){const x=qs[i];prog.textContent=`QUESTION ${i+1} / ${qs.length}`;scoreEl.textContent=`score: ${score}`;q.innerHTML=`<b>${i+1}.</b> ${x[0]}`;ans.innerHTML=x[1].map((a,j)=>`<button data-q="${j}">${a}</button>`).join('');res.textContent='';$$('#quizAnswers button').forEach(b=>b.addEventListener('click',()=>{const ok=+b.dataset.q===x[2];if(ok){score++;res.textContent='Correct. The fake science is thriving. 💘'}else res.textContent='Incorrect. Hossen’s lawyers have been notified. 😂';scoreEl.textContent=`score: ${score}`;$$('#quizAnswers button').forEach(z=>z.disabled=true);setTimeout(()=>{i++;if(i<qs.length)render();else{res.innerHTML=`<b>FINAL RESULT: ${score}/7</b><br>Compatibility: ${score>=5?'dangerously high.': 'still high because the website is biased.'}`;opened.add('quiz');updateDone();}},700)}))}render()}
function closeModal(){$('#modal').classList.remove('show');document.body.style.overflow=''}$('#closeModal').addEventListener('click',closeModal);$('.modal-backdrop').addEventListener('click',closeModal);document.addEventListener('keydown',e=>{if(e.key==='Escape'){closePhotoLightbox();closeModal()}});
