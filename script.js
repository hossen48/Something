...
function startMusic(){
  ...
  musicOn=true; $('#musicPill').classList.add('playing'); $('#musicLabel').textContent='soundtrack on · brighter';
  audioCtx=audioCtx||new(window.AudioContext||window.webkitAudioContext)();
  audioCtx.resume();
  const master=audioCtx.createGain(); master.gain.value=.14; master.connect(audioCtx.destination);
  // A brighter, more audible original melody — no external song file required.
  ...
  const tick=()=>{
    ...
    g.gain.exponentialRampToValueAtTime(.035,now+.035);
    g.gain.exponentialRampToValueAtTime(.0001,now+.48);
    o.connect(g).connect(master); o.start(now); o.stop(now+.52);
