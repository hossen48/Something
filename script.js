...
function startMusic(){
  ...
  $('#musicLabel').textContent='soundtrack on · LOUD ♫';
  audioCtx=audioCtx||new(window.AudioContext||window.webkitAudioContext)();
  audioCtx.resume();

  // Louder original soundtrack with melody + warm harmony.
  // Kept generated locally so the site needs no external audio file.
  const master=audioCtx.createGain();
  master.gain.value=.28;
  master.connect(audioCtx.destination);

  const notes=[261.63,329.63,392,523.25,493.88,392,329.63,293.66,349.23,440,523.25,440,392,329.63,293.66,261.63];
  const bass=[130.81,164.81,196,261.63,246.94,196,164.81,146.83];
  let i=0, b=0;

  const playTone=(freq,when,dur,gainValue,type='triangle')=>{
    const o=audioCtx.createOscillator(), g=audioCtx.createGain();
    o.type=type; o.frequency.value=freq;
    g.gain.setValueAtTime(.0001,when);
    g.gain.exponentialRampToValueAtTime(gainValue,when+.025);
    g.gain.exponentialRampToValueAtTime(.0001,when+dur);
    o.connect(g).connect(master); o.start(when); o.stop(when+dur+.03);
  };

  const tick=()=>{
    if(!musicOn)return;
    const now=audioCtx.currentTime;
    playTone(notes[i++%notes.length],now,.52,.075,'triangle');
    playTone(bass[b++%bass.length],now,.72,.035,'sine');
    // Soft fifth underneath every other beat.
    if(i%2===0) playTone(notes[(i+3)%notes.length]*1.5,now,.34,.018,'sine');
    melodyTimer=setTimeout(tick,570);
  };
  tick();
}
function toggleMusic(){
  if(!musicOn){startMusic();return}
  musicOn=false; $('#musicPill').classList.remove('playing'); $('#musicLabel').textContent='tap for brighter soundtrack'; clearTimeout(melodyTimer); audioCtx?.suspend();
}
$('#musicBtn').addEventListener('click',e=>{e.stopPropagation();toggleMusic()});document.addEventListener('pointerdown',()=>{if(!musicOn)startMusic()},{once:true});
...
let adventurePage=1;
const adventureInfo={
  1:['PAGE 1 · THE BEGINNING','Start soft. Then get suspicious.'],
  2:['PAGE 2 · THE MIDDLE','Memories, evidence, and a little chaos.'],
  3:['PAGE 3 · THE END… OR IS IT?','One test left. Then the final message.']
};
function showAdventurePage(n,animate=true){
  adventurePage=n;
  $$('.adventure-page').forEach(p=>p.classList.toggle('active',+p.dataset.adventurePage===n));
  $$('.adventure-dot').forEach(b=>b.classList.toggle('active',+b.dataset.adventure===n));
  const info=adventureInfo[n];
  if($('#adventureTitle'))$('#adventureTitle').textContent=info[0];
  if($('#adventureSubtitle'))$('#adventureSubtitle').textContent=info[1];
  if($('#adventureCount'))$('#adventureCount').textContent=`0${n} / 03`;
  if($('#adventureProgress'))$('#adventureProgress').textContent=`0${n}`;
  if(animate)sparkleBurst(10);
}
$$('.adventure-dot').forEach(b=>b.addEventListener('click',()=>showAdventurePage(+b.dataset.adventure)));
